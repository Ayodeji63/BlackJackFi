...
contract CasinoFiToken is ERC20, Ownable {
    ...
    uint256 public constant TREASURY_SUPPLY = (20 * MAX_SUPPLY) / 100; // 55% for treasury
    uint256 public constant CASINOFI_POOL = (30 * MAX_SUPPLY) / 100;
    uint256 public constant DEVELOPER_REWARDS_SUPPLY = (4 * MAX_SUPPLY) / 100; // 4% for developers
    uint256 public constant STAKING_REWARDS_SUPPLY = (6 * MAX_SUPPLY) / 100; // 1% for staking rewards
    ...
    function mintCasinoFiBetPool(address casinofiBet) external onlyOwner {
        _mint(casinofiBet, CASINOFI_POOL);
    }
    // Other functions...
    ...
    function burnToken(address account, uint256 value) external {
        _burn(account, value);
    }
}